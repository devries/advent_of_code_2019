...
int main(int argc, char *argv[]) {
  ...
  int *travel_distance_one;
  int *travel_distance_two;
  ...
  while((position_key=(point*)hashtable_iterator_get_key(iterator))!=NULL) {
    if((travel_distance_two=(int*)hashtable_get(grid_two, position_key, pt_size))!=NULL) {
      travel_distance_one=(int*)hashtable_get(grid_one, position_key, pt_size);

      distance = *travel_distance_one+*travel_distance_two;
  ...
  printf("Shortest travel distance: %d\n", min_distance);
}
...
void build_grid(hashtable *grid, char *path) {
  ...
  while((token = strsep(&path, ",")) != NULL) {
    ...
    for(i=0; i<distance; i++) {
      ...
      hashtable_insert(grid, &position, pt_size, &travel_distance, sizeof(int));
    }
  }
}
